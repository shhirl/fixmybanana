...
- [ ] **Two PRs waiting for Shirley, merge in this order: #9 (`how-its-built`, the page + eval data + takeaways + scale section), then #10 (`nav-link-how-its-built`, header link + eval line under the result card; stacked on #9). #7 can be closed after #10, it duplicates its footer change.** As of 2026-09-08 19:20 neither is merged; the live URL is 404 until #9 deploys. Design option B (editorial). Sections: Problem / Trade-offs / Solution, vision-model explainer, v0 + v2 blocks with the real eval numbers (60% photo-level agreement, 83% consistency, κ 0.12, $0.0046/analysis), cost & limits, ops, next steps, practice map. Numbers are read at request time from `eval/results/summary.json`, `v0-taxonomy.json`, `eval/v0/raw_response.json`, `labels.csv`.
  After merge: (1) check https://www.fixmybanana.com/how-its-built on desktop + phone; (2) `git tag -a look-2026-09-08-how-its-built -m "..." && git push origin --tags`; (3) `after-` screenshot set into `docs/screenshots/2026-09-08-after-how-its-built/`; (4) tiny PR `nav-link-how-its-built`: header link on `/` and `/upload` + one line under the result card "Evaluated on 54 photos · agrees with me 60% · see how"; (5) whirleyworld.com card + LinkedIn post linking `/how-its-built#v2` with one honest number.
  Then v1 (structured output per `docs/plans/fixmybanana-prompt-and-schema.md`), re-run `eval/run_eval.py --version v1 --runs 5`, `eval/summarize.py --version v1 --md --json`, add a v1 block.
