...
- [ ] **Next: v1 structured output.** Spec: `docs/plans/fixmybanana-prompt-and-schema.md` (strict JSON schema: photo_usable, view, support, banana_score 0–10, five segments, confidence + reason). Steps: branch `v1-structured-output`; new classifier call with `response_format: json_schema` alongside the old one behind a flag; `result.html` redesign for score/label/segments/confidence; keep `eval/run_eval.py` working (it imports the site's function, so extend the recorded fields: score, confidence, view, photo_usable); run `--version v1 --runs 5`; `summarize.py --version v1 --md --json`; add the v1 block + a v0→v1 before/after table to `/how-its-built` (numbers via `summary.json['v1']`); PR; tag `look-<date>-v1`.
  Also pending: LinkedIn post + whirleyworld.com card linking `/how-its-built#takeaways`; close PR #7 (superseded by #10).
...
- **2026-09-08 — `/how-its-built` shipped (PRs #8, #9, #10; tag `look-2026-09-08-how-its-built`).** Fixed the credits outage (logging + friendlier errors), built the 54-photo test set (CC + 3 derived + 5 synthetic), Shirley labelled it, ran v0 eval (270 analyses, $1.24: 60% photo-level agreement, 83% consistency, κ 0.12, 42/235 "unclear" runs), wrote the page (problem / trade-offs / solution / takeaways, vision-model explainer, v0 + v2 blocks, cost & limits, ops, scale, practice map), header link + eval line on the tool pages. After-screenshots in `docs/screenshots/2026-09-08-after-how-its-built/`.
