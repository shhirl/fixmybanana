...
- [ ] **🔴 Live site analysis is broken (seen 2026-09-08). Fix is PR #8 (`fix-vision-errors`) — Shirley merges, then uploads one photo on the live site; if it still fails, Railway → Deploy logs now show the real OpenAI status/body (likely `insufficient_quota` → check the OpenAI billing cap).** Background: Uploading a handstand photo returns "All Vision Models Failed. Please Try Again." (screenshot notes in `docs/screenshots/2026-09-08-baseline/README.md`). The key check against `/v1/models` passes, so the key is valid; all three chat-completions calls (`gpt-4o`, `gpt-4-turbo`, `gpt-4-turbo-2024-04-09`) return non-200 and `app.py` silently swallows the status/body (loop around line 218 — no logging). Most likely: OpenAI billing hard cap / insufficient_quota, or deprecated model ids. Next step: (1) check OpenAI dashboard usage + billing limits; (2) on a branch, log `response.status_code` + `response.text[:300]` per failed model and surface a friendlier error, so Railway logs show the real cause; (3) consider replacing the model list with current ids. This is a behavior change → branch + PR.
