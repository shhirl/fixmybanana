...
- [ ] **`/how-its-built` page is built — PR #9 (`how-its-built`) awaits Shirley's merge.** Design option B (editorial). Sections: Problem / Trade-offs / Solution, vision-model explainer, v0 + v2 blocks with the real eval numbers (60% photo-level agreement, 83% consistency, κ 0.12, $0.0046/analysis), cost & limits, ops, next steps, practice map. Numbers are read at request time from `eval/results/summary.json`, `v0-taxonomy.json`, `eval/v0/raw_response.json`, `labels.csv`.
